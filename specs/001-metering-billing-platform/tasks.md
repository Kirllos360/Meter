...
- [X] T023 [P] [US1] Contract test `assignMeter` (200 + 409 conflict) in `backend/test/contract/meter-assign.contract.spec.ts`
  - **Dependencies**: T012
...
- [X] T024 [P] [US1] Contract test `terminateMeter` + `getSimEligibility` in `backend/test/contract/meter-terminate.contract.spec.ts`
  - **Dependencies**: T012
...
- [X] T026 [P] [US1] Integration test — SIM reuse after meter termination in `backend/test/integration/sim-reuse.spec.ts`
  - **Dependencies**: T014
...
- [X] T027 [P] [US1] Projects module (CRUD + status + tax/threshold config) in `backend/src/projects/`
...
- [X] T028 [P] [US1] Locations module (CRUD over LocationNode hierarchy) in `backend/src/projects/locations/`
  - **Dependencies**: T013, T009
...
- [X] T029 [P] [US1] Customers module + CustomerUnitAssignment in `backend/src/customers/`
  - **Dependencies**: T013, T009
...
- [X] T030 [P] [US1] Meters module (CRUD + status lifecycle) in `backend/src/meters/`
...
- [X] T031 [P] [US1] SIM module (CRUD + eligibility endpoint) in `backend/src/sim-cards/`
  - **Dependencies**: T014, T009
...
- [X] T032 [US1] Assignment command + `POST /api/v1/meters/{meterId}/assign` (transactional) in `backend/src/meters/`
...
- [X] T033 [US1] Termination command + `POST /api/v1/meters/{meterId}/terminate` (SIM reuse) in `backend/src/meters/`
...
- [X] T034 [US1] Dashboard summary endpoints in `backend/src/projects/dashboard/`
...
- [X] T043 [P] [US2] Contract test `createReading` (201 + 422) in `backend/test/contract/readings.contract.spec.ts`
  - **Dependencies**: T012
...
- [X] T044 [P] [US2] Contract test `listReadingReviewQueue` in `backend/test/contract/review-queue.contract.spec.ts`
  - **Dependencies**: T012
...
- [X] T045 [P] [US2] Integration test — reading validation thresholds + suspicious flagging in `backend/test/integration/reading-validation.spec.ts`
  - **Dependencies**: T015
...
- [X] T046 [P] [US2] Project threshold-profile config in `backend/src/projects/thresholds/`
...
- [X] T047 [US2] Readings module + `POST /api/v1/readings` (consumption calc + validation) in `backend/src/readings/`
...
- [X] T048 [US2] Review queue `GET /api/v1/readings/review-queue` in `backend/src/readings/`
  - **Dependencies**: T047
  - **Area/Files**: `backend/src/readings/readings.controller.ts`, `readings.service.ts` (listReviewQueue)
  - **Acceptance**: Lists pending/suspicious readings with optional projectId/status filters; makes T044 pass
  - **Validation**: `cd backend && npm test -- review-queue.contract`
...
- [X] T053 [P] [US3] Contract test `generateInvoices` + `issueInvoice` (200 + 409 approval) in `backend/test/contract/invoices.contract.spec.ts`
  - **Dependencies**: T012
  - **Area/Files**: `backend/test/contract/invoice-generate.contract.spec.ts`, `invoice-issue.contract.spec.ts`
  - **Acceptance**: Asserts `InvoiceGenerateRequest`, generate `202`, `Invoice` schema, and `409` high-risk-approval envelope
  - **Validation**: `cd backend && npm test -- invoice-generate.contract invoice-issue.contract`
  - **Risk**: Generate is async-style (202 batch); assert batch shape.
  - **Note**: Tests pass against stub endpoints (T062/T063 implementations pending).

- [X] T054 [P] [US3] Contract test `addInvoiceAdjustment` in `backend/test/contract/adjustments.contract.spec.ts`
  - **Dependencies**: T012
  - **Area/Files**: `backend/test/contract/invoice-adjustment.contract.spec.ts`
  - **Acceptance**: Asserts `InvoiceAdjustmentRequest` + `201`
  - **Validation**: `cd backend && npm test -- invoice-adjustment.contract`
