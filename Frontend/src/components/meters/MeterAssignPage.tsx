...
import { mockProjects, mockBuildings, mockUnits, mockCustomers, mockMeters, mockSimCards } from '@/lib/mock-data';
...
export default function MeterAssignPage() {
  ...
  const filteredBuildings = form.project ? mockBuildings.filter((b) => b.projectId === form.project) : [];
  const selectedBuilding = mockBuildings.find((b) => b.id === form.building);
  const buildingUnits = form.building ? mockUnits.filter((u) => u.buildingId === form.building) : [];
  const floorUnits = form.floor ? buildingUnits.filter((u) => u.floorNumber === Number(form.floor)) : [];
  const floors = [...new Set(buildingUnits.map((u) => u.floorNumber))].sort((a, b) => a - b);
  const selectedUnit = mockUnits.find((u) => u.id === form.unit);
  const unitCustomers = selectedUnit?.customerId ? [mockCustomers.find((c) => c.id === selectedUnit.customerId)!].filter(Boolean) : mockCustomers.filter((c) => c.projectId === form.project);
  const availableMeters = form.meterType ? mockMeters.filter((m) => m.meterType === form.meterType && m.status === 'available') : [];
  const availableSims = mockSimCards.filter((s) => s.status === 'available' || s.status === 'reusable');
  ...
  const project = mockProjects.find((p) => p.id === form.project);
  const meter = mockMeters.find((m) => m.id === form.meter);
  const customer = mockCustomers.find((c) => c.id === form.customer);
  const sim = mockSimCards.find((s) => s.id === form.sim);
          ...
          {step === 2 && (
            <div className="space-y-3">
              ...
              <Select value={form.floor || ''} onValueChange={(v) => { setForm({ ...form, floor: v, unit: '' }); }}>
                ...
                <SelectContent>
                  {floors.map((f) => <SelectItem key={String(f)} value={String(f)}>Floor {f}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}
          {step === 3 && (
            <div className="space-y-3">
              ...
              <Select value={form.unit || ''} onValueChange={(v) => setForm({ ...form, unit: v })}>
                ...
                <SelectContent>
                  {floorUnits.map((u) => <SelectItem key={u.id} value={u.id}>{u.unitNumber} ({u.unitType})</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}
          {step === 4 && (
            <div className="space-y-3">
              ...
              <Select value={form.customer || ''} onValueChange={(v) => setForm({ ...form, customer: v })}>
                ...
                <SelectContent>
                  {unitCustomers.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <Select value={form.meterType || ''} onValueChange={(v) => { setForm({ ...form, meterType: v, meter: '' }); }}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="main_water">Main Water</SelectItem>
                  <SelectItem value="child_water">Child Water</SelectItem>
                </SelectContent>
              </Select>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              {availableMeters.length > 0 ? (
                <Select value={form.meter || ''} onValueChange={(v) => setForm({ ...form, meter: v })}>
                  ...
                  <SelectContent>
                    {availableMeters.map((m) => <SelectItem key={m.id} value={m.id}>{m.serialNumber} ({m.brand} {m.model})</SelectItem>)}
                  </SelectContent>
                </Select>
            ...
            <div className="space-y-3">
              ...
              {availableSims.length > 0 ? (
                <Select value={form.sim || ''} onValueChange={(v) => setForm({ ...form, sim: v })}>
                  ...
                  <SelectContent>
                    {availableSims.map((s) => <SelectItem key={s.id} value={s.id}>{s.msisdn} ({s.provider})</SelectItem>)}
                  </SelectContent>
                </Select>
